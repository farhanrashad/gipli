...
class AdmissionTeam(models.Model):
    ...
    def _get_default_team_id(self, user_id=None, domain=None):
        ...
        default_team = self.env['oe.admission.team'].browse(
            self.env.context['default_team_id']
        ) if self.env.context.get('default_team_id') else self.env['oe.admission.team']
        ...
        team = self.env['oe.admission.team']
        teams = self.env['oe.admission.team'].search([
        ...
        if not team:
            teams = self.env['oe.admission.team'].search([('company_id', 'in', valid_cids)])
    ...
    favorite_user_ids = fields.Many2many(
        'res.users', 'admission_team_favorite_user_rel', 'admission_team_id', 'user_id',
    ...
    def _compute_member_ids(self):
        for team in self:
            team.member_ids = team.admission_team_member_ids.user_id
    ...
    def _inverse_member_ids(self):
        for team in self:
            ...
            memberships = team.admission_team_member_ids
            users_current = team.member_ids
            users_new = users_current - memberships.user_id

            # add missing memberships
            self.env['oe.admission.team.member'].create([{'admission_team_id': team.id, 'user_id': user.id} for user in users_new])
    ...
    def _compute_member_warning(self):
        ...
        for team in self:
            ...
            other_memberships = self.env['oe.admission.team.member'].search([
                ('admission_team_id', '!=', team.id if team.ids else False),  # handle NewID
            ...
            if other_memberships and len(other_memberships) == 1:
                member_warning = _("Adding %(user_name)s in this team would remove him/her from its current team %(team_name)s.",
                                   ...
                                   team_name=other_memberships.admission_team_id.name
                                  )
                ...
                member_warning = _("Adding %(user_names)s in this team would remove them from their current teams (%(team_names)s).",
                                   ...
                                   team_names=", ".join(other_memberships.mapped('admission_team_id.name'))
                                  )
            if member_warning:
    ...
    def _search_member_ids(self, operator, value):
        return [('admission_team_member_ids.user_id', operator, value)]
    ...
    def create(self, vals_list):
        teams = super(AdmissionTeam, self.with_context(mail_create_nosubscribe=True)).create(vals_list)
        teams.filtered(lambda t: t.member_ids)._add_members_to_favorites()
        return teams

    def write(self, values):
        res = super(AdmissionTeam, self).write(values)
        # manually launch company sanity check
        if values.get('company_id'):
            self.admission_team_member_ids._check_company(fnames=['admission_team_id'])
