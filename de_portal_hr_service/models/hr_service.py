...
class HRService(models.Model):
    ...
    header_model_id = fields.Many2one('ir.model', ondelete='cascade', string='Model', required=True)
    ...
    allow_log_note = fields.Boolean(string='Allow Log Note', store=True, compute='_compute_allow_log_note', readonly=False, help='Allow Log Note to user on portal')

    show_attachment = fields.Boolean(string='Show Attachments', store=True, compute='_compute_show_attachment', readonly=False, help='show attachment on portal')

    show_activities = fields.Boolean(string='Show Activities', store=True, compute='_compute_show_activities', readonly=False, help='show activities on portal')
    ...
    def _get_domain(self, domain):
        filter_domain = []
        if domain:
            filter_domain = safe_eval.safe_eval(domain)
        return filter_domain

    def _get_current_record_with_domain(self, record, domain_str):
        domain = []
        if record:
            domain = self._get_domain(domain_str)
            combined_domain = expression.AND([domain, [('id', '=', record.id)]])
            record_id = self.env[self.header_model_id.model].search(combined_domain)
            #record.search(domain)
            return record_id
