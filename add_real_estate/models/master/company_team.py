...
class CompanyTeam(models.Model):
    ...
    name = fields.Char('Company Team', required=True, translate=True)
    ...
    user_id = fields.Many2one('res.partner', string='Team Leader')
    member_ids = fields.One2many(
        'res.partner', 'company_team_id', string='Channel Members', check_company=True,
    ...
    def create(self, values):
        team = super(CompanyTeam, self.with_context(mail_create_nosubscribe=True)).create(values)
        if values.get('member_ids'):
            team._add_members_to_favorites()
        return team

    def write(self, values):
        res = super(CompanyTeam, self).write(values)
        if values.get('member_ids'):
    ...
    def unlink(self):
        ...
        return super(CompanyTeam, self).unlink()
