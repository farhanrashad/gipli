...
class PaymentStrgRequest(models.Model):
    ...
    payments_ids = fields.Many2many(comodel_name="account.payment", relation="payment_str_request", column1="p", column2="s", string="", )
    ...
    def onchange_currency_id(self):
        if self.currency_id:
            if self.currency_id.id !=self.env.company.currency_id.id:
                self.amount=self.amount_currency*self.currency_id.rate
    ...
    def _compute_amount_due(self):
        for rec in self:
            ...
            if rec.amount_due == 0.0 and rec.base_amount>0:
                rec.is_pay = True
            else:
                rec.is_pay =False
    ...
    def create(self, values):
        print("values :1> ",values['state_payment'])
...
class PaymentStrg(models.Model):
    ...
    invoice_id = fields.Many2one(comodel_name='account.move',string='Installment')
    installment_state = fields.Selection([
                        ...
                        ('invoicing_legacy', 'Invoicing App Legacy'),
                          ], readonly=True, default='not_paid', copy=False, string="Installment State",compute="_compute_is_state_payment")
    ...
    def _compute_is_state_payment(self):
        for rec in self:
            if rec.force_posted:
                ...
                if rec.invoice_id:
                    rec.state = rec.invoice_id.state
                else:
                    rec.state = 'draft'
            rec.installment_state=rec.invoice_id.payment_state
    ...
    def _compute_amount_due(self):
            for rec in self:
                ...
                if rec.amount_due == 0.0 and  rec.amount>0:
                    rec.is_pay = True
                else:
                    rec.is_pay =False
