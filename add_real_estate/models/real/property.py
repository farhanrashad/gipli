...
class ProductProduct(models.Model):
    ...
    state = fields.Selection([('draft', _('Draft')),
                              ('request_available', _('Request Available')),
                              ('approve', _('Approve')),
                              ('available', _('Available')),
                              ('temp_reservation', _('Temp Reservation')),
                              ('requested', _('Requested')),
                               ('reserved', _('Reserved')),
                              ('initial_contracted', _('Initial Contracted')),
                              ('contracted', _('Installments'),),
                               ('blocked', _('Blocked')),
                               ('pricing', _('Pricing')),
                              ('exception',_('Exception'))], string="Status", default='draft', copy=False, tracking=True)
    is_contracted = fields.Boolean(string="is Contracted", compute="_compute_is_contract" )
    is_contracted_draft = fields.Boolean(string="is Contracted Draft", compute="_compute_is_contract_draft")
    ...
    def convert_to_pricing(self):
        if self.state=='blocked':
            raise ValidationError("Not Allow Convert From Blocked To Pricing")
        self.state='pricing'
    ...
    def _compute_is_contract_draft(self):
        for rec in self:
            rec.is_contracted_draft = True
            account_move_line = self.env['account.move.line'].search([('product_id', '=', rec.id),
                                                          ], limit=1)
            if account_move_line.move_id.state == 'draft':
                rec.state = "initial_contracted"
    ...
    finish_of_property = fields.Selection(string="Finishing Type", selection=[('Finish', 'Finish'), ('Core&Shell', 'Core&Shell'), ], required=False)
    ...
    unit_price_lines = fields.One2many(comodel_name="unit.price.lines", inverse_name="unit_id", string="Unit Price Factors", required=False)

    @api.onchange('unit_price_lines','is_factor_price_unit')
    def onchange_is_factor_price_unit_4(self):
        if self.is_factor_price_unit and self.unit_price_lines:
            res=self.unit_price_lines.filtered(lambda factor:factor.factor_id.is_finish and factor.factor_id.finish_of_property_id )
            if res:
                self.finish_of_property_id=res[0].factor_id.finish_of_property_id



    @api.onchange('project_id')
    def onchange_project_id40(self):
        print(":ff:::")
        for rec in self:
            lines=[]
            rec.unit_price_lines=[(5, 0, 0)]
            for line in rec.project_id.unit_price_lines:
                lines.append((0,0,{
                    'factor_id':line.factor_id.id,
                    'space':line.space,
                    'price':line.price,
                    'is_print':line.is_print,
                }))
            rec.unit_price_lines=lines
    ...
    discount_value = fields.Float(string="Amount", required=False,track_visibility = 'always')
    change_price_lines = fields.One2many(comodel_name="unit.price.changes", inverse_name="unit_id", string="", required=False)
    final_unit_price = fields.Float(string="Final Unit Price ",  required=False, compute="_compute_final_unit_price",track_visibility = 'onchange')
    final_unit_price2 = fields.Float(string="Final Unit Price ",track_visibility = 'always')

    def _compute_final_unit_price(self):
        for rec in self:
            summ=0
            for line in rec.unit_price_lines:
                summ+=line.total

            if rec.unit_price_lines:
                rec.final_unit_price = summ
            else:
            ...
            if rec.discount_type=='amount':
                rec.final_unit_price=rec.final_unit_price-rec.discount_value
            if rec.discount_type=='percentage':
                rec.final_unit_price=rec.final_unit_price-((rec.discount_value/100)*rec.final_unit_price)
            rec.final_unit_price2=rec.final_unit_price
    ...
    def write(self, vals):
        ...
        if 'price_m' in vals:
            history_unit_price = self.env['history.property.unit.price'].create({
                ...
                'price_m': self.unit_price2 / self.sellable if self.sellable else self.unit_price2 /1,
        ...
        if 'price_finishing_for_m' in vals:
            history_finishing_price = self.env['history.property.finishing.price'].create({
                ...
                'is_finish': self.is_finish,
                'finish_of_property_id': self.finish_of_property_id.id,
                'price_finishing_for_m': self.finishing_price2 / self.sellable if self.sellable else self.finishing_price2 ,
                'design_of_property_id': self.design_of_property_id.id,
                'is_pool': self.is_pool,
                'price_pool_for_one': self.price_pool_for_one,
                'number_of_pool': self.number_of_pool,
        ...
        if 'price_garage_for_one' in vals:
            history_garage_price = self.env['history.property.garage.price'].create({
                ...
                'design_of_property_id': self.design_of_property_id.id,
                'is_pool': self.is_pool,
                'price_garage_for_one': self.garage_price2/self.number_of_garage if self.number_of_garage else self.garage_price2/ 1,
                'number_of_pool': self.number_of_pool,
                'price_profile': self.price_profile,
            })
    ...
    def action_view_factor_line_price(self):
        self.ensure_one()
        action = self.env.ref('add_real_estate.action_factor_line_price').read()[0]
        action['domain'] = [
            ('unit_id', '=', self.id),
        ]
        print("action %s",action)
    ...
    def _compute_view_button_create(self):
        for rec in self:
            ...
            res = self.env['res.reservation'].search(
                [('property_id', '=', rec.id),("state",'in',['draft','contracted','initial_contracted','reserved'])
                                                          ])
            if len(req) > 0:
                rec.is_req_res = True
            else:
