        ...
        if self.reservation_id_2:
            ...
            for line in self.reservation_id_2.payment_strg_ids:
                    for line2 in line.payments_ids:
                        if (line2.state in  ['under_coll','posted'] and line.state_payment=='cheque') :
                            ids.append(line2.id)
    ...
    def onchange_method_reservation_id(self):
        if self.reservation_id:
            ...
            for line in self.reservation_id.payment_strg_ids:
                    for line2 in line.payments_ids:
                        if (line2.state == 'collected' and line.state_payment=='cheque') or (line2.state == 'posted' and line.state_payment!='cheque'):
                            ids.append(line2.id)
        ...
        for rec in self:
            ...
            for under in payment_under_coll:
                ...
                for refunded in payment_refunded_under_collection:
                    refunded.move_date = fields.Date.today()
                    ...
                    for line in rec.reservation_id_2.payment_strg_ids:
                        if line.id in ids_p:
                            ...
                            for li in line.payments_ids:
                                ids.append(li.id)
                                if  (li.state in  ['under_coll','posted'] and line.state_payment=='cheque' and li.id in rec.payments_post_res1_ids.ids):
