...
class PurchaseRequisitionLine(models.Model):
    ...
    @api.onchange('cost_account_id','product_id','requisition_id.analytic_account_id')
    def _onchange_cost_account_id(self):
        ...
        if self.requisition_id.analytic_account_id:
            # Add the project analytic account (if exists) with 100% (1.0)
            distribution[self.requisition_id.analytic_account_id.id] = 100
