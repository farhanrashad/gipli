...
class hr_payslip_employees_inherit(models.TransientModel):
    ...
    def compute_sheet(self):
        ...
        contract = []
        contracts = self.employee_ids._get_contracts(payslip_run.date_start, payslip_run.date_end, states=['open', 'close','cancel'])
        for rec in self.employee_ids:
            employee_contracts = self.env['hr.contract'].search([('employee_id','=',rec.id),('id','in',contracts.ids)])
            for item in employee_contracts:
                if item.state == 'open':
                    contract.append(item.id)
                    break
                elif item.state == 'close':
                    contract.append(item.id)
                    break
                elif item.state == 'cancel':
                    contract.append(item.id)
                    break
        contracts = self.env['hr.contract'].search([('id','in',contract)])
