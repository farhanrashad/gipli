...
patch(SaleOrderLineProductField.prototype, 'de_school_library', {
    ...
    _editLineConfiguration() {
        this._super(...arguments);
        if (this.props.record.data.is_borrow_order) {
            this._openRentalConfigurator(true);
        }
    },

    get isConfigurableLine() {
        return this._super(...arguments) || !!this.props.record.data.is_borrow_order;
    },
    ...
    _defaultRentalData: function (edit) {
        ...
        if (edit) {
            ...
            const saleOrderLines = saleOrderRecord.data.order_line.records.filter(
                line => !line.data.display_type && line.data.is_product_rentable && line.data.is_borrow_order
            );
            let defaultPickupDate, defaultReturnDate;

            if (saleOrderLines.length) {
                saleOrderLines.forEach(function (line) {
                    if (line.data.is_borrow_order) {
    ...
    async _openRentalConfigurator(edit) {
        this.action.doAction(
            'de_school_library.action_library_fee_config_wizard',
            {
