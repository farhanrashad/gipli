...
class AccountPayment(models.Model):
    ...
    @api.depends('payment_reconcile_line','payment_reconcile_line.amount_assigned_recon',
                 'payment_reconcile_debit_line.amount_assigned',
                 'payment_reconcile_credit_line.amount_assigned',
                 'amount')
    def _amount_all(self):
        for payment in self:
            
            amount_debit_currency = 0
            amount_credit_currency = 0
            amount_debit_recon = 0
            amount_credit_recon = 0.0
            amount = 0
            amount_currency = 0
            ...
            amount_debit_currency =  sum(payment.payment_reconcile_debit_line.filtered(lambda x:x.assigned).mapped('amount_assigned_currency'))
            amount_debit_recon = sum(payment.payment_reconcile_debit_line.filtered(lambda x:x.assigned).mapped('amount_assigned_recon'))

            amount_credit_currency = sum(payment.payment_reconcile_credit_line.filtered(lambda x:x.assigned).mapped('amount_assigned_currency'))
            amount_credit_recon = sum(payment.payment_reconcile_credit_line.filtered(lambda x:x.assigned).mapped('amount_assigned_recon'))



            
            amount_currency = payment.currency_id._convert(abs(payment.amount), payment.company_currency_id, payment.company_id, payment.date)
            amount = payment.amount
            
            if payment.payment_type == 'inbound':
                amount_credit_currency = sum(payment.payment_reconcile_credit_line.filtered(lambda x:x.assigned).mapped('amount_assigned_currency')) + amount_currency
                amount_credit_recon = sum(payment.payment_reconcile_credit_line.filtered(lambda x:x.assigned).mapped('amount_assigned_recon')) + amount
            elif payment.payment_type == 'outbound':
                #amount_debit_currency = amount_debit_currency + amount_currency
                amount_debit_currency =  sum(payment.payment_reconcile_debit_line.filtered(lambda x:x.assigned).mapped('amount_assigned_currency')) + amount_currency
                amount_debit_recon = sum(payment.payment_reconcile_debit_line.filtered(lambda x:x.assigned).mapped('amount_assigned_recon')) + amount
    ...
    def action_generate_journal_entry(self):
        ...
        for payment in self:
            ...
            if not (payment.currency_id.id == payment.company_id.currency_id.id):
                ...
                amount = payment.currency_id._convert(abs(payment.amount), payment.company_id.currency_id, payment.company_id, payment.date)                    
            ...
            default_line_name = self.env['account.move.line']._get_default_line_name(_("Internal Transfer") if self.is_internal_transfer else payment_display_name['%s-%s' % (payment.payment_type, self.partner_type)], payment.amount, payment.currency_id, payment.date, partner=self.partner_id,)
            
            debit_line_name = ' - '.join(recon.move_id.name + ' ' + str(payment.currency_id.symbol) + ' ' + "{:,.2f}".format(recon.amount_assigned_recon) for recon in payment.payment_reconcile_debit_line.filtered(lambda x: x.assigned))
            credit_line_name = ' - '.join(recon.move_id.name + ' ' + str(payment.currency_id.symbol) + ' ' + "{:,.2f}".format(recon.amount_assigned_recon) for recon in payment.payment_reconcile_credit_line.filtered(lambda x: x.assigned))
            ...
            if payment.payment_type == 'inbound':
                ...
                temp_text_to_display += 'debit=' + str(round(payment.amount_total_signed,2))
            ...
            if payment.payment_type == 'outbound':
                ...
                temp_text_to_display += ',credit=' + str(round(payment.amount_total_signed,2))
            # --------------------------------------------
            ...
            for line in payment.payment_reconcile_debit_line:
                if line.assigned:
                    ...
                    temp_text_to_display += ',Assigned Credit=' + str(round(line.amount_assigned_currency,2))
            ...
            for line in payment.payment_reconcile_credit_line:
                if line.assigned:
                    ...
                    temp_text_to_display += ',Assigned debit=' + str(round(line.amount_assigned_currency,2))
            # --------------------------------------------
            ...
            if payment.amount_diff_currency > 0:
                ...
                total_debit_diff = 0
                total_credit_diff = abs(payment.amount_diff_currency)
                
            if payment.amount_diff_currency != 0 and payment.writeoff_account_id:
                line_ids.append([0,0,{
                ...
                temp_text_to_display += ', Exchange Debit=' + str(round(total_debit_diff,2))
                temp_text_to_display += ',Exchange Credit=' + str(round(total_credit_diff,2))
            #raise ValidationError(_(temp_text_to_display))
    ...
    def action_reconcile(self):
        for payment in self:
            ...
            if len(payment.payment_reconcile_debit_line.filtered(lambda x: x.assigned)) or len(payment.payment_reconcile_credit_line.filtered(lambda x: x.assigned)):
                ...
                # --------------------------------------------
                if payment.payment_type == 'outbound':
                    payment_line = payment.move_id.line_ids.filtered(lambda line: line.account_id.id == payment.destination_account_id.id)
                    payment_debit_line = payment_line
                    payment_credit_line = payment.matching_move_id.line_ids.filtered(lambda line: line.account_id.id == payment.destination_account_id.id and line.payment_matching)
                elif payment.payment_type == 'inbound':
                    payment_line = payment.move_id.line_ids.filtered(lambda line: line.account_id.id == payment.destination_account_id.id)
                    payment_credit_line = payment_line
                    payment_debit_line = payment.matching_move_id.line_ids.filtered(lambda line: line.account_id.id == payment.destination_account_id.id and line.payment_matching)
                if payment.amount > 0 :
                ...
                if payment.payment_type == 'outbound' or payment.payment_type == 'inbound':
                    ...
                    for line in payment.payment_reconcile_debit_line.filtered(lambda line: line.assigned == True):
                        ...
                        if credit_line:
                            ...
                            .reconcile()
                    for line in payment.payment_reconcile_credit_line.filtered(lambda line: line.assigned == True):
    ...
    def _compute_all_exchange(self):
        ...
        if not self.currency_id.id == self.company_currency_id.id:
            ...
            exchange_rate = self.currency_id._convert(1, self.company_id.currency_id, self.company_id, self.date)                    
            ...
            if move:
                ...
                if move.recon_currency_id.id == move.company_currency_id.id:
                    ...
                    if move.move_line_id.amount_residual != 0:
                        move.amount_residual_recon = abs(move.currency_id._convert(abs(move.move_line_id.amount_residual), move.recon_currency_id, move.move_line_id.company_id, move.payment_id.date))
                    else:
                        move.amount_residual_recon = 0

                    move.amount_total_recon = abs(move.currency_id._convert(abs(move.move_line_id.debit - move.move_line_id.credit), move.recon_currency_id, move.move_line_id.company_id, move.payment_id.date))
            #except:
            #    move.amount_total = 0
