...
class EnrolReport(models.Model):
    ...
    name = fields.Char('Enrollment Ref.', readonly=True)
    date = fields.Datetime('Enrollment Date', readonly=True)
    ...
    enrol_status = fields.Selection([
        ...
        ('drop', 'Dropped'),
        ('cancel', 'Cancelled'),
    ], string="Enroll Status", readonly=True)
    ...
    order_id = fields.Many2one('sale.order', 'Enrollment #', readonly=True)
