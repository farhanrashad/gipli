...
class SaleTaxRegister(models.AbstractModel):
    ...
    def _get_report_values(self, docids ,data=None):
        ...
        if docs.target_move == 'posted':
            invoices = self.env['account.move'].search([('invoice_date', '>=', docs.start_date),('invoice_date_date', '<=', docs.end_date),('journal_id.type','=', 'sale'),('state','=', 'posted')])
        else:
            invoices = self.env['account.move'].search([('invoice_date_date', '>=', docs.start_date),('invoice_date_date', '<=', docs.end_date),('journal_id.type','=', 'sale')])
