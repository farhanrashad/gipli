...
class CustomerPortal(CustomerPortal):
    ...
    def _get_service_records_labels_after(self,service_id):
        template =  super(CustomerPortal, self)._get_service_records_labels_after(service_id)
        template += '<th></th>'
        return template
    
    def _get_service_records_row_cols_after(self,service_id, record_id):
        template =  super(CustomerPortal, self)._get_service_records_row_cols_after(service_id, record_id)
        col = False
        domain = []
        # Action Items
        service_sudo = request.env['hr.service'].search([('id','=',service_id.id)],limit=1)
